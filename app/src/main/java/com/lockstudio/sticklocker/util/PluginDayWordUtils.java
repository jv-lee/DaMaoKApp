...
public class PluginDayWordUtils implements OnClickListener,TwoWayAbsListView.OnScrollListener {
	...
	private OnGravityChange onGravityChange;
	private ImageView gravity_left_imageview, gravity_center_imageview, gravity_right_imageview, gravity_top_imageview, gravity_bottom_imageview;
	private TwoWayGridView gv_text_font;

	private SeekBar sb_plugin_size;
		...
		public boolean handleMessage(Message msg) {
			...
			case 2:
				...
				if (!TextUtils.isEmpty(plugin_fontPath) && new File(plugin_fontPath).exists()) {
					onPluginSettingChange.change(plugin_fontPath, plugin_fontUrl, plugin_size, plugin_color, plugin_shadowColor);
				}
				break;
			case 3:
	...
	public PluginDayWordUtils(Context mContext) {
		this.mContext = mContext;
		view = LayoutInflater.from(mContext).inflate(R.layout.plugin_text_layout, null);
		...
		gravity_left_imageview = (ImageView) view.findViewById(R.id.gravity_left_imageview);
		gravity_center_imageview = (ImageView) view.findViewById(R.id.gravity_center_imageview);
		gravity_right_imageview = (ImageView) view.findViewById(R.id.gravity_right_imageview);
		gravity_top_imageview = (ImageView) view.findViewById(R.id.gravity_top_imageview);
		gravity_bottom_imageview = (ImageView) view.findViewById(R.id.gravity_bottom_imageview);
		gravity_left_imageview.setOnClickListener(this);
		gravity_center_imageview.setOnClickListener(this);
		gravity_right_imageview.setOnClickListener(this);
		gravity_top_imageview.setOnClickListener(this);
		gravity_bottom_imageview.setOnClickListener(this);

		view.findViewById(R.id.gravity_layout).setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		} else if (i == R.id.gravity_left_imageview) {
			setGravity(0);
			onGravityChange.gravityChange(0);

		} else if (i == R.id.gravity_center_imageview) {
			setGravity(1);
			onGravityChange.gravityChange(1);

		} else if (i == R.id.gravity_right_imageview) {
			setGravity(2);
			onGravityChange.gravityChange(2);

		} else if (i == R.id.gravity_top_imageview) {
			setGravity(3);
			onGravityChange.gravityChange(3);

		} else if (i == R.id.gravity_bottom_imageview) {
			setGravity(4);
			onGravityChange.gravityChange(4);

		} else {
		}
	}

	public void updateGravityImageview() {
		if (orientation == 0) {
			gravity_left_imageview.setEnabled(true);
			gravity_right_imageview.setEnabled(true);
			gravity_center_imageview.setEnabled(true);
			gravity_top_imageview.setEnabled(false);
			gravity_bottom_imageview.setEnabled(false);

		} else {
			gravity_left_imageview.setEnabled(false);
			gravity_right_imageview.setEnabled(false);
			gravity_center_imageview.setEnabled(false);
			gravity_top_imageview.setEnabled(true);
			gravity_bottom_imageview.setEnabled(true);
		}
	}
	...
	public void setOnGravityChange(OnGravityChange onGravityChange) {
		this.onGravityChange = onGravityChange;
	}

	public void setScale(float max, float mScale) {
		plugin_size = mScale;
		sb_plugin_size.setMax((int) (max * 100));
		sb_plugin_size.setProgress((int) (mScale * 100));
	}
	...
	public interface OnPluginSettingChange {
		public abstract void change(String fontPath, String fontUrl, float size, int color, int shadowColor);
	}

	public interface OnGravityChange {
		public abstract void gravityChange(int gravity);
	}
	@Override
