...
import com.lockstudio.sticklocker.view.NumPasswordLockView;
import com.lockstudio.sticklocker.view.NumPasswordLockView.OnLockSettingListener;
import com.lockstudio.sticklocker.view.NumPasswordLockView.State;
...
public class CreateNumPasswordLockActivity extends BaseActivity implements OnUpdateViewListener, OnLockSettingListener,OnClickListener {
	private NumPasswordLockView mPassnumLockView;
...
//		cleanPass.setOnClickListener(this);
		...
		NumPasswordLockerInfo wordPasswordLockerInfo = (NumPasswordLockerInfo) themeConfig.getLockerInfo();
		...
		mPassnumLockView = (NumPasswordLockView) LayoutInflater.from(mContext).inflate(R.layout.numpassword_locker_layout, null);
		mPassnumLockView.setPassword(true);
		mPassnumLockView.setContainerLayoutParams(mLayoutParams);
		mPassnumLockView.setOnUpdateViewListener(this);
		mPassnumLockView.setVisible(false);
		mPassnumLockView.setEditable(false);
		mPassnumLockView.setPasswordIndView(passwordIndView);
		mPassnumLockView.setTextView(cleanPass);
		mPassnumLockView.setPassnumLockerInfo(wordPasswordLockerInfo);
		mPassnumLockView.setOnLockSettingListener(this);

		lockContainer.addView(mPassnumLockView, mLayoutParams);
	...
	public void updateState(State state, String confirmPassword) {
		...
		if (State.ConfirmWrong == state) {
			...
			updateState(State.First, "");
			mPassnumLockView.updateState(State.First);
			return;
		}
		if (State.Done == state) {
	...
	public void onClick(View arg0) {
		...
		if(mPassnumLockView!=null){
			mPassnumLockView.cleanPassword();
		}
	}
