...
import com.lockstudio.sticklocker.view.LockPatternView_12.Cell;
import com.lockstudio.sticklocker.view.LockPatternView_12.DisplayMode;
 ...
 * 
 */
public class CreateGesture12PasswordActivity extends BaseActivity implements OnClickListener, OnUpdateViewListener {
	private LockPatternView_12 mLockPatternView_12;
	private TextView mHeaderText;
	private List<Cell> mChosenPattern = null;
	...
	private void showToast(CharSequence message) {
		SimpleToast.makeText(mContext, message, SimpleToast.LENGTH_SHORT).show();
	}
	...
	private void initViewAndEvent() {
		...
		twelvePatternLockerInfo = (TwelvePatternLockerInfo) themeConfig.getLockerInfo();
		...
		mLockPatternView_12 = (LockPatternView_12) LayoutInflater.from(mContext).inflate(R.layout.twelve_pattern_locker_layout, null);
		mLockPatternView_12.setContainerLayoutParams(mLayoutParams);
		mLockPatternView_12.setOnUpdateViewListener(this);
		mLockPatternView_12.setVisible(false);
		mLockPatternView_12.setTwelvePatternLockerInfo(twelvePatternLockerInfo);
		mLockPatternView_12.setOnPatternListener(mChooseNewLockPatternListener);
		mLockPatternView_12.setTactileFeedbackEnabled(LockApplication.getInstance().getConfig().isVibrate());
		mLockPatternView_12.updateImage();
		lockContainer.addView(mLockPatternView_12, mLayoutParams);
		...
		canvas_line_togglebutton.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				drawLine = isChecked;
				twelvePatternLockerInfo.setDrawLine(drawLine);
			}
		});
		...
		drawLine = twelvePatternLockerInfo.isDrawLine();
		if (drawLine) {
		...
		lineColor = twelvePatternLockerInfo.getLineColor();
		updateImageView();

	}
	public void updateImageView() {
		twelvePatternLockerInfo.setLineColor(lineColor);
	...
	private Runnable mClearPatternRunnable = new Runnable() {
		public void run() {
			mLockPatternView_12.clearPattern();
		}
	};

	protected LockPatternView_12.OnPatternListener mChooseNewLockPatternListener = new LockPatternView_12.OnPatternListener() {

		public void onPatternStart() {
			mLockPatternView_12.removeCallbacks(mClearPatternRunnable);
			patternInProgress();
		}
	...
	private void updateStage(Stage stage) {
		...
		mLockPatternView_12.setDisplayMode(DisplayMode.Correct);
		...
		case ChoiceTooShort:
			...
			mLockPatternView_12.setDisplayMode(DisplayMode.Wrong);
...
//			postClearPatternRunnable();
	...
	private void postClearPatternRunnable() {
		mLockPatternView_12.removeCallbacks(mClearPatternRunnable);
		mLockPatternView_12.postDelayed(mClearPatternRunnable, 1000);
	}
	...
	private void saveChosenPatternAndFinish() {
		...
		new LockPatternUtils_12(mContext).saveLockPattern(mChosenPattern);
