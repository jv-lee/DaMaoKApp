 ...
 * 
 */
public class FlashlightStickerView extends View implements OnPluginSettingChange {
	/**
	 * 图片的最大缩放比例
	 */
	...
	private FlashlightStickerInfo mFlashlightStickerInfo;
	...
	public FlashlightStickerView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		this.mContext = context;
	}

	public FlashlightStickerView(Context context) {
		this(context, null);
		this.mContext = context;
	}

	public FlashlightStickerView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void adjustLayout() {
		...
		if (mViewPaddingLeft != newPaddingLeft || mViewPaddingTop != newPaddingTop) {
			...
			mFlashlightStickerInfo.x = mViewPaddingLeft + mDrawableWidth / 2 + framePadding;
			mFlashlightStickerInfo.y = mViewPaddingTop + mDrawableHeight / 2 + framePadding;
	...
	private void transformDraw() {
		...
		mFlashlightStickerInfo.angle = (int) mDegree;
		mFlashlightStickerInfo.scale = mScale;
		mFlashlightStickerInfo.textSize = (int) (textSize * mScale);
		invalidate();
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mStatus == STATUS_DELETE && isVisiable) {
				mOnRemoveViewListener.removeView(mFlashlightStickerInfo, this);
				isEditable = false;
				mStatus = STATUS_INIT;
				return true;
			}
...
//			this.mContext.registerReceiver(mBatInfoReceiver, new IntentFilter(Intent.ACTION_BATTERY_CHANGED));
	...
	public void setFlashlightStickerInfo(FlashlightStickerInfo flashlightStickerInfo) {

		this.mFlashlightStickerInfo = flashlightStickerInfo;
		textSize = DensityUtil.dip2px(mContext, 10);
		if (mFlashlightStickerInfo.textSize == 0) {
			mFlashlightStickerInfo.textSize = DensityUtil.dip2px(mContext, 10);
		}

		mScale = mFlashlightStickerInfo.scale;
		if (mScale >= MAX_SCALE) {
			mScale = MAX_SCALE;
		}
		imageRes = R.drawable.plugin_flashlight;
		
		getNewBitMap();

		Matrix matrix = new Matrix();
		matrix.postRotate(mFlashlightStickerInfo.angle, mBitmap.getWidth() / 2, mBitmap.getHeight() / 2);
		Bitmap b = Bitmap.createBitmap(mBitmap, 0, 0, mBitmap.getWidth(), mBitmap.getHeight(), matrix, true);
		mCenterPoint.x = mFlashlightStickerInfo.x + b.getWidth() / 2.0f;
		mCenterPoint.y = mFlashlightStickerInfo.y + b.getHeight() / 2.0f;
		mDegree = flashlightStickerInfo.angle;
		init();
		if (isVisiable) {
			showControllerView();
		}
			...
			public void onClick(View v) {
				...
				try {
					if(isFlashlightOn()){
						...
						imageRes = R.drawable.plugin_flashlight_press;
					}
					flashlightUtils();
					getNewBitMap();
					invalidate();
	...
	private void showControllerView() {
		if (controllerView == null) {
			...
			pluginSettingUtils.initSelectData(mFlashlightStickerInfo.textColor);
	...
	private void getNewBitMap() {
		...
		Drawable db = DrawableUtils.getDrawableCustomColor(mContext, bitmapRes, mFlashlightStickerInfo.textColor);
		Bitmap bp = null;
		if (db != null) {
		...
		bp = DrawableUtils.scaleTo(bp, (int) (mFlashlightStickerInfo.width * mScale), (int) (mFlashlightStickerInfo.height * mScale));
		int width = bp.getWidth();
		int height = bp.getHeight();

		mTextPaint = new Paint();
		mTextPaint.setAntiAlias(true);
		mTextPaint.setColor(mFlashlightStickerInfo.textColor);
	...
	public void change(int color) {
		mFlashlightStickerInfo.textColor = color;
		getNewBitMap();
		transformDraw();
	}
