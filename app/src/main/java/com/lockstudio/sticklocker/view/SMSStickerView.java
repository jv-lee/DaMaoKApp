 ...
 * 
 */
public class SMSStickerView extends View implements OnPluginSettingChange {
	/**
	 * 图片的最大缩放比例
	 */
	...
	private SMSStickerInfo mSMSStickerInfo;
	...
	public SMSStickerView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		this.mContext = context;
	}

	public SMSStickerView(Context context) {
		this(context, null);
		this.mContext = context;
	}

	public SMSStickerView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void adjustLayout() {
		...
		if (mViewPaddingLeft != newPaddingLeft || mViewPaddingTop != newPaddingTop) {
			...
			mSMSStickerInfo.x = mViewPaddingLeft + mDrawableWidth / 2 + framePadding;
			mSMSStickerInfo.y = mViewPaddingTop + mDrawableHeight / 2 + framePadding;
	...
	private void transformDraw() {
		...
		mSMSStickerInfo.angle = (int) mDegree;
		mSMSStickerInfo.scale = mScale;
		invalidate();
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mStatus == STATUS_DELETE && isVisiable) {
				mOnRemoveViewListener.removeView(mSMSStickerInfo, this);
				isEditable = false;
				mStatus = STATUS_INIT;
				return true;
			}
	...
	private BroadcastReceiver mScreenBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_SCREEN_ON.equals(action)) {
				UpdateUnreadMmsSms(mContext,false);
			}
		}
	};
	...
	// 设置系统时间并实时更新
	public void setSMSStickerInfo(SMSStickerInfo smsStickerInfo) {

		IntentFilter filter = new IntentFilter();
		filter.addAction(Intent.ACTION_SCREEN_ON);
		this.mContext.registerReceiver(mScreenBroadcastReceiver, filter);

		UpdateUnreadMmsSms(mContext,true);

		this.mSMSStickerInfo = smsStickerInfo;
		mScale = mSMSStickerInfo.scale;
		getNewBitMap();
		Matrix matrix = new Matrix();
		matrix.postRotate(mSMSStickerInfo.angle, mBitmap.getWidth() / 2, mBitmap.getHeight() / 2);
		Bitmap b = Bitmap.createBitmap(mBitmap, 0, 0, mBitmap.getWidth(), mBitmap.getHeight(), matrix, true);
		mCenterPoint.x = mSMSStickerInfo.x + b.getWidth() / 2.0f;
		mCenterPoint.y = mSMSStickerInfo.y + b.getHeight() / 2.0f;
		mDegree = smsStickerInfo.angle;
		init();
		if (isVisiable) {
			showControllerView();
		}
	...
	private void showControllerView() {
		if (controllerView == null) {
			...
			pluginSettingUtils.initSelectData(mSMSStickerInfo.textColor);
	...
	private void getNewBitMap() {
		Drawable db = DrawableUtils.getDrawableCustomColor(mContext, R.drawable.plugin_sms, mSMSStickerInfo.textColor);
		Bitmap bp = null;
		if (db != null) {
		...
		if (bp == null) {
			bp = DrawableUtils.getBitmap(mContext, R.drawable.plugin_sms);
		}
		bp = DrawableUtils.scaleTo(bp, (int) (mSMSStickerInfo.width * mScale), (int) (mSMSStickerInfo.height * mScale));
		int width = bp.getWidth();
		int height = bp.getHeight();

		int textSize1 = (int) (height / 2 - DensityUtil.dip2px(mContext, 8));
		mTextPaint = new Paint();
		mTextPaint.setAntiAlias(true);
		mTextPaint.setColor(mSMSStickerInfo.textColor);
		...
		if (SMS_NUM != 0) {
			canvas.drawText(SMS_NUM + "", width / 2, baseline, mTextPaint);
		}
	}
	...
	public void change(int color) {
		mSMSStickerInfo.textColor = color;
		getNewBitMap();
		transformDraw();
	}
