...
class SearchLabelRepository @Inject
constructor(service: APIService) : BaseRepository<APIService>(service) {

    fun getSearchLabels(page: Int, value: String, type: Int): LiveData<Data<LabelEntity>> {
        val data = MutableLiveData<Data<LabelEntity>>()
        service.getSearchLabels(page, Constants.PAGE_NUMBER, value, type, Config.imei).enqueue(object : Callback<Response<Data<LabelEntity>>> {
