...
import static com.yuan7.lockscreen.utils.ConstUtil.DAY;
import static com.yuan7.lockscreen.utils.ConstUtil.HOUR;
import static com.yuan7.lockscreen.utils.ConstUtil.MIN;
import static com.yuan7.lockscreen.utils.ConstUtil.MSEC;
import static com.yuan7.lockscreen.utils.ConstUtil.SEC;
import static com.yuan7.lockscreen.utils.ConstUtil.TimeUnit;
...
public class TimeUtil {
    ...
    public static final SimpleDateFormat DEFAULT_SDF_DATE = new SimpleDateFormat("MM/dd", Locale.getDefault());
    public static final SimpleDateFormat DEFAULT_SDF_TIME = new SimpleDateFormat("HH:mm", Locale.getDefault());
    ...
    public static String getChineseTimeMill(long time) {
        ...
        if (timeLong < MIN) {
            ...
            return milliseconds2String(time, new SimpleDateFormat("MM-dd"));
        }
    }

    public static String getChineseTimeString(String strTime) {
        long time = string2Milliseconds(strTime);
        long timeLong = System.currentTimeMillis() - time;
        if (timeLong < MIN) {
            return "刚刚";
        } else if (timeLong < HOUR) {
    ...
    public static String getChineseTimeString3(String strTime) {
        long time = string2Milliseconds(strTime);
        long timeLong = System.currentTimeMillis() - time;
        if (timeLong < MIN) {
            return "刚刚";
            ...
            return milliseconds2String(time, new SimpleDateFormat("HH:mm"));
        }
    }


    public static String getTimeStringSimple(String time) {
        long time1 = Long.valueOf(time);
        long time2 = (System.currentTimeMillis() - time1);
        if (time2 < DAY) {
            return milliseconds2String(time1, new SimpleDateFormat("HH:ss"));
        } else {
            return milliseconds2String(time1);
        }
    ...
    public static String getChineseTimeString2(String time) {
        long timeLong = TimeUtil.string2Milliseconds(time);
        long timeLong2 = (System.currentTimeMillis() - timeLong);
        if (timeLong2 < MIN) {
            return "刚刚";
        } else if (timeLong2 <= HOUR) {
            return timeLong2 / MIN + " 分钟";
        } else if (time.substring(0, 10).equals(getCurTimeString().substring(0, 10))) {
            return time.substring(11, 16);
        } else if (timeLong2 <= (DAY * 2)) {
            return "昨天";
        } else if (timeLong2 <= (DAY * 7)) {
            return getWeek(time);
        } else {
    ...
    public static boolean isShowTime(String time) {
        ...
        if (timeList.size() == 0) {
            ...
            long timeLong1 = TimeUtil.string2Milliseconds(timeList.get(0));
            long timeLong2 = TimeUtil.string2Milliseconds(time);

            //新增时间减去时间头 小于20分钟内 则添加至集合 返回false 不显示时间头
            if ((timeLong2 - timeLong1) <= (MIN * 20)) {
                timeList.add(time);
                return false;
